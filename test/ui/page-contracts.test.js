...
test('library page provides search, type, tag filters, and detail content', () => {
  ...
  for (const marker of ['library-search', 'library-type', 'library-tag', 'ingredients', 'steps']) assert.ok(source.includes(marker), marker);
...
test('tongue page exposes safe assisted recording and explicit state actions', () => {
  ...
  for (const marker of ['type="file"', 'observations', 'doctorConclusion', 'confirmedTags', '/confirm', '/archive', '/restore']) assert.ok(source.includes(marker), marker);
