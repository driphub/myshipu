...
export async function renderTongue({ mount, api, members, taxonomy, labels, showToast }) {
  ...
  async function load() {
    ...
    list.innerHTML = records.length ? records.map((record) => {
      ...
      return `<article class="record-card"><div class="record-photo">${record.photoPath ? `<img src="${escapeHtml(mediaUrl(record.photoPath))}" alt="${escapeHtml(member?.name || '')}的舌苔记录">` : '<span>无照片</span>'}</div><div class="record-body"><div class="record-meta"><span class="status status-${escapeHtml(record.status)}">${statusLabel(record.status)}</span><span>${escapeHtml(record.observedAt)}</span></div><h2>${escapeHtml(member?.name || '已删除成员')}</h2><p>${escapeHtml(observations)}</p><blockquote>${escapeHtml(record.doctorConclusion || '尚未录入专业医生结论')}</blockquote><div class="tag-list">${record.confirmedTags.map((tag) => `<span>${escapeHtml(labels[tag] || tag)}</span>`).join('')}</div></div><div class="record-actions"><button class="button secondary" data-action="edit" data-id="${escapeHtml(record.id)}">编辑</button>${record.status === 'draft' ? `<button class="button primary" data-action="confirm" data-id="${escapeHtml(record.id)}">确认用于推荐</button>` : ''}${record.status === 'active' ? `<button class="button secondary" data-action="archive" data-id="${escapeHtml(record.id)}">归档</button>` : ''}${record.status === 'archived' ? `<button class="button secondary" data-action="restore" data-id="${escapeHtml(record.id)}">恢复</button>` : ''}<button class="icon-button danger" data-action="delete" data-id="${escapeHtml(record.id)}" aria-label="删除记录">×</button></div></article>`;
