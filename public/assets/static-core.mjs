...
export function memberSafetyFlags(member) {
...
export function isEligible(item, member) {
...
export function scoreItemForMember(item, member, season, confirmedTags = []) {
...
export function rankPlanPairs({ members, recipes, teas, date, confirmedTagsByMember = {} }) {
  const safeRecipes = (recipes || []).filter((candidate) => members.every((member) => isEligible(candidate, member)));
  const safeTeas = (teas || []).filter((candidate) => members.every((member) => isEligible(candidate, member)));
  const missing = [];
