...
export async function renderTongue({ mount, api, members, taxonomy, labels, showToast }) {
  ...
  mount.querySelector('#new-tongue').addEventListener('click', () => dialog.showModal());
  mount.querySelectorAll('[data-close]').forEach((button) => button.addEventListener('click', () => dialog.close()));
  mount.querySelector('#tongue-form').addEventListener('submit', async (event) => {
    ...
    body.append('memberId', form.elements.memberId.value);
    ...
    try {
      await api('/api/tongue-records', { method: 'POST', body });
      dialog.close();
      form.reset();
      showToast('舌象草稿已保存', 'success');
      await load();
    } catch (error) { showToast(error.message, 'error'); }
  });
  await load();
